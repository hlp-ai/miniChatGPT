...
# - Rewrite the code to match other transformer style in this repo
# - Add utilties function to LLaMA class
...
class Attention(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        self.cache_k = torch.zeros((args.max_batch_size, args.max_seq_len,
                                    self.n_heads, self.head_dim)).cuda()
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor,
        ...
        output = torch.matmul(scores,
                              values)    # (bs, n_heads, slen, head_dim)
